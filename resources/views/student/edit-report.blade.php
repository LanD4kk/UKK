...
<body class="bg-background-light dark:bg-background-dark text-[#0d121b] dark:text-gray-100 min-h-screen flex flex-col">
    ...
    <header class="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-sm">
        <div class="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div class="hidden sm:flex items-center gap-1.5 text-xs font-medium text-gray-400">
                <span class="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xs font-bold"><span class="material-symbols-outlined text-[16px]">edit</span></span>
                <span>Perbarui detail laporan</span>
            </div>
        </div>
    </header>
    ...
    <div id="success-overlay" class="hidden fixed inset-0 z-50 bg-white dark:bg-background-dark flex flex-col items-center justify-center text-center px-8">
        ...
        <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white mb-2">Perubahan Disimpan!</h2>
        <p class="text-gray-500 dark:text-gray-400 text-sm mb-8 max-w-xs">
            Laporan Anda telah berhasil diperbarui dan siap untuk ditinjau oleh admin.
        </p>
    ...
    <script>
        ...
        async function loadExistingReport() {
            try {
                ...
                // Jika bukan pending block form
                if (complaint.status !== 'Pending') {
                    showToast('Laporan tidak dapat diedit karena status sudah diproses.');
                    setTimeout(() => window.location.href = `/student/report/{{ $id }}`, 2000);
                    return;
                }

                // Populate data
                document.getElementById('category_id').value = Array.from(document.getElementById('category_id').options).find(o => o.text === complaint.category)?.value || '';
                titleInput.value = complaint.title;
                titleInput.dispatchEvent(new Event('input')); // trigger counter
                
                descInput.value = complaint.description;
                descInput.dispatchEvent(new Event('input')); // trigger counter

                if (complaint.evidence_photo) {
                    previewImg.src = complaint.evidence_photo;
                    photoNameEl.textContent = 'Foto saat ini (Biarkan jika tidak ingin diganti)';
